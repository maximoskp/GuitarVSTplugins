...
DelayVSTAudioProcessor::createParameterLayout()
{
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;

    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{"highf", 1},
        "HF",
        juce::NormalisableRange<float>(50.0f, 2000.0f, 1.0f),
        80.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{"lowf",1},
        "LF",
...
void DelayVSTAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    delays.resize(getTotalNumInputChannels());
        
        for (auto& d : delays)
            d.setSampleRate(sampleRate);   // if you have prepare()
    
    float highf = *parameters.getRawParameterValue("highf");
    float lowf = *parameters.getRawParameterValue("lowf");
    ...
    for (auto& delay : delays){
        delay.setHPFfrequency(highf);
        delay.setHPFfrequency(lowf);
        delay.setDelayTime(time);
        delay.setFeedback(feedback);
        delay.setWetDry(wet, 1.0f);
    }
    
    parameters.addParameterListener("highf", this);
    parameters.addParameterListener("lowf", this);
...
void DelayVSTAudioProcessor::parameterChanged(const juce::String& id, float newValue)
{
    if (id == "highf"){
        for (auto& delay : delays) delay.setHPFfrequency(newValue);
    }else if(id == "lowf"){
        for (auto& delay : delays) delay.setLPFfrequency(newValue);
    }else if(id == "time"){
        for (auto& delay : delays) delay.setDelayTime(newValue);
    }else if(id == "feedback"){
        for (auto& delay : delays) delay.setFeedback(newValue);
    }else if(id == "wet"){
        for (auto& delay : delays) delay.setWetDry(newValue, 1.);
    }
}
...
void DelayVSTAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        auto& delayChannel = delays[channel];

        // ..do something to the data...
        for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
        {
            channelData[sample] = delayChannel.process_sample(channelData[sample]);
        }
    }
}
